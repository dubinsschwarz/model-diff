...
"""Compute Attempt 001 merged-minus-ablated activation differences."""
...
FROZEN_SELF_DIFF_SPEC = {
    ...
    "input_information_policy": (
        "canonical_merged_attempt001_ablations_and_frozen_generic_probe_only"
    ),
...
def validate_attempt_inputs(
    ...
    epsilons = self_diff_spec["epsilons"]
    block_index = self_diff_spec["transformer_block_index"]
    ...
    if attempt_spec.get("transformer_block_index") != block_index:
    ...
    if method.get("transformer_block_index") != block_index:
        raise ValueError("Construction manifest block mismatch")
    if method.get("epsilons") != epsilons:
        raise ValueError("Construction manifest epsilon mismatch")
    if method.get("stored_dtype") != "float32":
        raise ValueError("Construction manifest checkpoint dtype mismatch")

    merged_entry = verify_checkpoint(
        merged_dir,
        construction.get("source_checkpoint"),
        "merged checkpoint",
    )

    defaults = attempt_spec.get("defaults")
    outputs = attempt_spec.get("outputs")
    if not isinstance(defaults, dict) or not isinstance(outputs, dict):
        raise ValueError("Attempt specification is missing checkpoint locations")
    output_root_value = defaults.get("output_root")
    directory_names = outputs.get("directory_names")
    ...
    if not isinstance(directory_names, dict):
        raise ValueError("Attempt specification has invalid directory names")
    output_root = Path(output_root_value).expanduser()

    output_entries = construction.get("output_checkpoints")
    if not isinstance(output_entries, list) or len(output_entries) != len(epsilons):
        raise ValueError("Construction manifest output checkpoint count mismatch")
    by_epsilon = {}
    for entry in output_entries:
        if not isinstance(entry, dict):
            raise ValueError("Malformed construction output checkpoint")
        epsilon = entry.get("epsilon")
        if isinstance(epsilon, bool) or epsilon not in epsilons:
            raise ValueError("Unexpected construction output epsilon")
        if epsilon in by_epsilon:
            raise ValueError(f"Duplicate construction output epsilon: {epsilon}")
        by_epsilon[epsilon] = entry

    ablated = []
    for epsilon in epsilons:
        entry = by_epsilon.get(epsilon)
        if entry is None:
            raise ValueError(f"Missing construction output epsilon: {epsilon}")
        key = f"{epsilon:.2f}"
        directory_name = directory_names.get(key)
        if (
            not isinstance(directory_name, str)
            or not directory_name
            or Path(directory_name).name != directory_name
            or entry.get("directory_name") != directory_name
        ):
            raise ValueError(f"Construction output directory mismatch: {epsilon}")
        checkpoint_dir = output_root / directory_name
        normalized = verify_checkpoint(
            checkpoint_dir,
            entry,
            f"ablated checkpoint epsilon {epsilon}",
        )
        ablated.append(
            {
                "epsilon": epsilon,
    ...
    return {
        ...
        "construction_manifest_sha256": sha256_file(
            construction_manifest_path
        ),
        "merged_checkpoint": merged_entry,
        "ablated_checkpoints": ablated,
    }
...
class ActivationAccumulator:
    """Accumulate block outputs without retaining per-example activations."""
...
def compute_checkpoint_mean(
    ...
    try:
        ...
        block_index = spec["transformer_block_index"]
        hook_handle = transformer.layers[block_index].register_forward_hook(
            accumulator
        )
...
def build_artifact(
    merged_mean64: Any,
    ablated_means64: list[tuple[float, Any]],
    hidden_size: int,
    spec: dict[str, Any],
    torch_module: Any,
) -> dict[str, Any]:
    ...
    by_epsilon = {}
    for epsilon, mean in ablated_means64:
        if epsilon in by_epsilon:
            raise ValueError(f"Duplicate ablated mean epsilon: {epsilon}")
        validate_float64_mean(
            mean,
            sequence_length,
            hidden_size,
        ...
        by_epsilon[epsilon] = mean
    if set(by_epsilon) != set(spec["epsilons"]):
        raise ValueError("Ablated mean epsilon set mismatch")

    stored_merged = merged_mean64.to(torch_module.float32).contiguous()
    ablations = {}
    for epsilon in spec["epsilons"]:
        ablated_mean64 = by_epsilon[epsilon]
        difference64 = merged_mean64 - ablated_mean64
        ablations[f"{epsilon:.2f}"] = {
            "epsilon": epsilon,
    ...
    return {
        ...
        "merged_mean": stored_merged,
        "ablations": ablations,
    }


def normalized_checkpoint_for_manifest(entry: dict[str, Any]) -> dict[str, Any]:
    normalized = normalize_checkpoint_entry(entry, "checkpoint manifest entry")
    return normalized
...
def build_manifest(
    ...
    ablated_checkpoints: list[dict[str, Any]],
    artifact_sha256: str,
    raw_hashes: dict[str, Any],
    hidden_size: int,
) -> dict[str, Any]:
    epsilon_order = {epsilon: index for index, epsilon in enumerate(spec["epsilons"])}
    checkpoint_epsilons = [item.get("epsilon") for item in ablated_checkpoints]
    if (
        len(checkpoint_epsilons) != len(spec["epsilons"])
        or set(checkpoint_epsilons) != set(spec["epsilons"])
    ):
        raise ValueError("Checkpoint epsilon set mismatch")
    normalized_ablated = []
    for checkpoint in sorted(
        ablated_checkpoints,
        key=lambda item: epsilon_order[item["epsilon"]],
    ):
        normalized = normalized_checkpoint_for_manifest(checkpoint)
        normalized_ablated.append(
            {
                "epsilon": checkpoint["epsilon"],
                "directory_name": checkpoint["directory_name"],
                **normalized,
            }
        )

    raw_ablations = raw_hashes.get("ablations")
    if not isinstance(raw_ablations, list):
        raise ValueError("Raw activation hashes are missing ablations")
    raw_epsilons = [item.get("epsilon") for item in raw_ablations]
    if (
        len(raw_epsilons) != len(spec["epsilons"])
        or set(raw_epsilons) != set(spec["epsilons"])
    ):
        raise ValueError("Raw activation hash epsilon set mismatch")
    normalized_raw = sorted(
        (dict(item) for item in raw_ablations),
        key=lambda item: epsilon_order[item["epsilon"]],
    )
    ...
    for item in normalized_raw:
        require_sha256(item.get("ablated_mean"), "ablated mean")
        require_sha256(item.get("difference"), "self-difference")
    ...
    return {
        ...
        "input_checkpoints": {
            "merged": normalized_checkpoint_for_manifest(merged_checkpoint),
            "ablated": normalized_ablated,
        },
        "probe": {
        ...
        "self_diff_artifact": {
            ...
            "raw_tensors_sha256": {
                ...
                "ablations": normalized_raw,
            },
        },
    }


def artifact_raw_hashes(artifact: dict[str, Any], torch_module: Any) -> dict[str, Any]:
    merged_hash = sha256_raw_float32_tensor(
        artifact["merged_mean"],
        torch_module,
    )
    ablations = []
    for key, record in artifact["ablations"].items():
        epsilon = record["epsilon"]
        if key != f"{epsilon:.2f}":
            raise ValueError("Artifact epsilon key mismatch")
        ablations.append(
            {
                "epsilon": epsilon,
                "ablated_mean": sha256_raw_float32_tensor(
                    record["ablated_mean"],
                    torch_module,
                ),
                "difference": sha256_raw_float32_tensor(
                    record["difference"],
                    torch_module,
                ),
            }
        )
    ablations.sort(key=lambda item: item["epsilon"])
    return {"merged_mean": merged_hash, "ablations": ablations}
...
def save_outputs(
    ...
    manifest = build_manifest(
        ...
        input_context["ablated_checkpoints"],
...
def compute_means(
    ...
    spec: dict[str, Any],
    torch_module: Any,
) -> tuple[Any, list[tuple[float, Any]], int]:
    ...
    ablated_means = []
    for checkpoint in input_context["ablated_checkpoints"]:
        mean, found_hidden_size = compute_checkpoint_mean(
        ...
        if found_hidden_size != hidden_size:
            raise ValueError("Ablated checkpoint hidden size mismatch")
        ablated_means.append((checkpoint["epsilon"], mean))
    return merged_mean, ablated_means, hidden_size
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        merged_mean, ablated_means, hidden_size = compute_means(
            args.merged_model_dir,
            input_context,
            probe,
            spec,
