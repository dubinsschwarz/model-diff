...
"""Evaluate Attempt 003 against the frozen post-blind-stage oracle."""
...
ATTEMPT_DIRECTORY = (
    PROJECT / "experiments" / "attempts" / "003_gaussian_noise_prefix0_13_r00125"
)
...
DEFAULT_CONSTRUCTOR_SCRIPT_PATH = (
    PROJECT / "scripts" / "ablation" / "construct_gaussian_noise.py"
)
DEFAULT_SELF_DIFF_SCRIPT_PATH = (
    PROJECT / "scripts" / "ablation" / "compute_self_diff_attempt003.py"
)
...
DEFAULT_SELF_DIFF_ARTIFACT_PATH = Path(
    "/root/model-diff-scratch/artifacts/attempt003_self_diff/self_diff.pt"
)
...
FROZEN_EVALUATION_SPEC = {
    ...
    "orientation_rationale": (
        "Gaussian noise has arbitrary global orientation; signed cosine remains "
        "primary for direct comparability and absolute cosine is a preregistered "
        "secondary measure that must not replace it."
    ),
    "geometry": {
        "dtype": "float64",
        "metrics": [
        ...
        "zero_vector_policy": {
            ...
            "absolute_cosine_similarity": (
                "null_if_either_vector_has_zero_norm"
            ),
    ...
    "evaluation_policy": {
        ...
        "signed_cosine_remains_primary": True,
        "absolute_cosine_is_preregistered_secondary": True,
        "automatic_ranking": False,
        "semantic_grading": False,
        "best_position_selection": False,
        "post_result_metric_selection": False,
        "attempt002_comparison": False,
        "gaussian_seed_selection_or_tuning": False,
    },
    "defaults": {
...
SELF_VECTOR_TYPES = ("merged_mean", "noisy_mean", "difference")
...
def validate_attempt_provenance(
    ...
    *,
    ...
    """Validate Attempt 003's complete frozen chain and canonical checkpoint."""
    attempt_spec = load_json_object(attempt_spec_path, "Attempt 003 specification")
    construction = load_json_object(
        construction_manifest_path, "Attempt 003 construction manifest"
    )
    self_diff_spec = load_json_object(
        self_diff_spec_path, "Attempt 003 self-difference specification"
    )
    self_manifest = load_json_object(
        self_diff_manifest_path, "Attempt 003 self-difference manifest"
    )
    ...
    expected_attempt_fields = {
        ...
        "transformer_block_indices": blocks,
    ...
    for key, expected in expected_attempt_fields.items():
        if attempt_spec.get(key) != expected:
    ...
    verify_manifest_file_link(
        construction, "spec_sha256", attempt_spec_path, "construction specification"
    )
    constructor_hash = verify_manifest_file_link(
        construction,
        "constructor_sha256",
    ...
    expected_construction_fields = {
        ...
        "seed": 1729,
        "target_relative_frobenius": 0.00125,
        "randomness": attempt_spec.get("randomness"),
        "weight_transform": attempt_spec.get("weight_transform"),
        "stored_dtype": "float32",
        "standalone_checkpoint": True,
    }
    for key, expected in expected_construction_fields.items():
        if construction_method.get(key) != expected:
    ...
    if not isinstance(ordering, dict) or not isinstance(modified, list):
        raise ValueError("Construction manifest is missing deterministic ordering")
    ...
    if (
        ...
        or ordering.get("generator_stream")
        != "single_cpu_stream_in_modified_matrix_order"
        or not isinstance(ordered_names, list)
        or len(ordered_names) != 98
        or ordered_names != sorted(ordered_names)
        or len(set(ordered_names)) != 98
    ...
    expected_self_spec = {
        ...
        "stored_dtype": "float32",
        "difference_sign": "merged_minus_noisy",
    }
    ...
    for key, expected in expected_self_spec.items():
        if key != "attempt_id" and method.get(key) != expected:
            raise ValueError(f"Self-difference manifest method mismatch: {key}")
    if method.get("hidden_size") != EXPECTED_SHAPE[1]:
        raise ValueError("Self-difference manifest hidden size mismatch")
    ...
    self_source = normalize_checkpoint_entry(
        inputs.get("merged"), "self-difference merged checkpoint"
    )
    ...
    construction_output = construction.get("output_checkpoint")
    self_noisy = inputs.get("noisy")
    if not isinstance(construction_output, dict) or not isinstance(self_noisy, dict):
        raise ValueError("Noisy checkpoint provenance is missing")
    if construction_output.get("directory_name") != "model" or self_noisy.get(
        "directory_name"
    ) != "model":
        raise ValueError("Noisy checkpoint directory mismatch")
    if normalize_checkpoint_entry(
        construction_output, "construction output checkpoint"
    ) != normalize_checkpoint_entry(
        self_noisy, "self-difference noisy checkpoint"
    ):
        raise ValueError("Noisy checkpoint provenance mismatch")
    ...
    return {
        ...
        "constructor_script_sha256": constructor_hash,
...
def geometry_metrics(
    ...
    return {
        ...
        "absolute_cosine_similarity": (
            None if cosine is None else abs(cosine)
        ),
...
def verify_float32_parameters(model: Any, torch_module: Any) -> None:
    ...
    unexpected = sorted(
        {str(parameter.dtype) for parameter in floating if parameter.dtype != torch_module.float32}
    )
    if unexpected:
        raise ValueError("Merged model has unexpected floating dtypes: " + ", ".join(unexpected))
    devices = {parameter.device.type for parameter in floating}
    if devices != {"cpu"}:
        raise ValueError(f"Merged model must be on CPU for Logit Lens, found {sorted(devices)}")


def validate_loaded_qwen3_model(
    model: Any, hidden_size: int, torch_module: Any
) -> tuple[Any, Any, int]:
    ...
    if final_norm is None or lm_head is None:
        raise ValueError("Merged Qwen3 model does not expose final norm and lm_head")
    norm_weight = getattr(final_norm, "weight", None)
    head_weight = getattr(lm_head, "weight", None)
    if not isinstance(norm_weight, torch_module.Tensor) or tuple(norm_weight.shape) != (hidden_size,):
        raise ValueError("Final model norm has unexpected dimensions")
    if not isinstance(head_weight, torch_module.Tensor) or tuple(head_weight.shape) != (vocab_size, hidden_size):
...
def load_local_model_and_tokenizer(
    ...
    try:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            merged_dir, dtype=torch_module.float32, local_files_only=True
        )
    ...
    *,
    ...
    return {
        ...
        "method": {
            ...
            "orientation_rationale": spec["orientation_rationale"],
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        model, tokenizer, final_norm, lm_head, vocab_size = load_local_model_and_tokenizer(
