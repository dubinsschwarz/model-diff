...
"""Construct Attempt 002 by BF16-rounding selected merged weights."""
...
DEFAULT_OUTPUT_ROOT = Path(
    "/root/model-diff-scratch/ablations/002_bf16_roundtrip_prefix0_13"
)
...
def bf16_roundtrip_weight(weight: Any, torch_module: Any) -> Any:
    if not isinstance(weight, torch_module.Tensor) or weight.ndim != 2:
...
def realized_perturbation_metrics(
    ...
    if (
        ...
        raise ValueError(
            "Perturbation inputs must be matching CPU float32 matrices"
        )
...
def apply_bf16_roundtrip(
    ...
    with torch_module.no_grad():
        for module_name, module in sorted(eligible, key=lambda item: item[0]):
            original32 = module.weight.detach()
            modified32 = bf16_roundtrip_weight(original32, torch_module)
...
def normalized_matrix_records(
    ...
    normalized = []
    seen = set()
    for record in sorted(records, key=lambda item: item["module_name"]):
        module_name = record.get("module_name")
        if not isinstance(module_name, str) or not module_name or module_name in seen:
...
def build_construction_manifest(
    ...
    combined_original_norm = math.sqrt(
        sum(record["original_frobenius_norm"] ** 2 for record in matrices)
    )
    ...
    return {
        ...
        "aggregate_perturbation": {
            ...
            "combined_realized_fp32_delta_frobenius_norm": combined_delta_norm,
...
def construct(
    ...
    eligible = discover_eligible_linear_weights(
        model,
        spec["transformer_block_indices"],
        torch,
    )
    matrix_records = apply_bf16_roundtrip(eligible, torch)
