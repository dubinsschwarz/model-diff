...
"""Evaluate Attempt 002 against the frozen post-blind-stage oracle."""
...
DEFAULT_CONSTRUCTOR_SCRIPT_PATH = (
    PROJECT / "scripts" / "ablation" / "construct_bf16_roundtrip.py"
)
...
SELF_VECTOR_TYPES = ("merged_mean", "ablated_mean", "difference")
...
def validate_attempt_provenance(
    ...
    *,
    ...
    """Validate Attempt 002's complete frozen chain and canonical checkpoint."""
    attempt_spec = load_json_object(attempt_spec_path, "Attempt 002 specification")
    construction = load_json_object(
        construction_manifest_path, "Attempt 002 construction manifest"
    )
    self_diff_spec = load_json_object(
        self_diff_spec_path, "Attempt 002 self-difference specification"
    )
    self_manifest = load_json_object(
        self_diff_manifest_path, "Attempt 002 self-difference manifest"
    )
    ...
    self_ablated = inputs.get("ablated")
    if not isinstance(construction_output, dict) or not isinstance(self_ablated, dict):
        raise ValueError("Ablated checkpoint provenance is missing")
    if construction_output.get("directory_name") != "model" or self_ablated.get(
        "directory_name"
    ) != "model":
    ...
    ) != normalize_checkpoint_entry(self_ablated, "self-difference ablated checkpoint"):
        raise ValueError("Ablated checkpoint provenance mismatch")
