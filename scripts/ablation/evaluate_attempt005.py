...
"""Evaluate Attempt 005 against the frozen post-blind-stage oracle."""
...
DEFAULT_CONSTRUCTOR_SCRIPT_PATH = (
    PROJECT / "scripts" / "ablation" / "construct_generic_gradient_rollback.py"
)
DEFAULT_SELF_DIFF_SCRIPT_PATH = (
    PROJECT / "scripts" / "ablation" / "compute_self_diff_attempt005.py"
)
DEFAULT_FREEZE_CORPUS_SCRIPT_PATH = (
    PROJECT / "scripts" / "ablation" / "freeze_generic_rollback_corpus.py"
)
...
FROZEN_EVALUATION_SPEC = {
    ...
    "evaluation_policy": {
        ...
        "post_result_strength_tuning": False,
        "alternative_alpha_selection": False,
        "prior_attempt_thresholds": False,
    },
    "defaults": {
...
SELF_VECTOR_TYPES = ("merged_mean", "rollback_mean", "difference")
...
FROZEN_ATTEMPT_HASHES = {
    "spec.json": "373674ebbfe94a169a7a658a3ba5b3b393f54c0926423838e71e1d4ea6d459d2",
    "corpus_spec.json": "e514820e7def99e9287c32de4b1279f28f6ff6c2e09749a8dfa5d8b14c311f0f",
    "corpus-manifest.json": "8b3ea49228dbb106c2b8a7c849c4f27e021fa3e992800444f43e260a6b055230",
    "construction-manifest.json": "7f84e0c46e49a658336a0944762bc38fe49e0afed38755dea711a1acc3c03e48",
    "self_diff_spec.json": "8df44769da356311aee178d250be03a714f1c6b1c8a485978091250598b85837",
    "self-diff-manifest.json": "4c32a226d7f302a2fa6853a64ed95f9f8abc0942c2a4a1fd87de06c070f927b2",
}
...
def validate_attempt_provenance(
    ...
    *,
    ...
    freeze_corpus_script_path: Path,
    constructor_script_path: Path,
    self_diff_script_path: Path,
    merged_hashes_path: Path,
) -> dict[str, Any]:
    paths = {
        "spec.json": attempt_spec_path,
        "corpus_spec.json": corpus_spec_path,
        "corpus-manifest.json": corpus_manifest_path,
        "construction-manifest.json": construction_manifest_path,
        "self_diff_spec.json": self_diff_spec_path,
        "self-diff-manifest.json": self_diff_manifest_path,
    }
    for filename, path in paths.items():
        verify_file_sha256(path, FROZEN_ATTEMPT_HASHES[filename], filename)
    attempt_spec = load_json_object(attempt_spec_path, "Attempt 005 specification")
    corpus_spec = load_json_object(corpus_spec_path, "Attempt 005 corpus specification")
    corpus_manifest = load_json_object(corpus_manifest_path, "Attempt 005 corpus manifest")
    construction = load_json_object(construction_manifest_path, "Attempt 005 construction manifest")
    self_spec = load_json_object(self_diff_spec_path, "Attempt 005 self-difference specification")
    self_manifest = load_json_object(self_diff_manifest_path, "Attempt 005 self-difference manifest")
    ...
    if (attempt_spec.get("format_version") != 1 or attempt_spec.get("attempt_id") != attempt_id or
        ...
        attempt_spec.get("eligible_tensors", {}).get("transformer_block_indices") != list(range(14)) or
        attempt_spec.get("eligible_tensors", {}).get("expected_matrix_count") != 98 or
        attempt_spec.get("rollback", {}).get("target_relative_frobenius") != 0.00125 or
        attempt_spec.get("rollback", {}).get("normalization") != "one_global_alpha_across_all_eligible_matrices"):
        raise ValueError("Attempt 005 frozen specification mismatch")
    if (corpus_spec.get("format_version") != 1 or corpus_spec.get("attempt_id") != attempt_id or
        corpus_spec.get("dataset", {}).get("repo_id") != "science-of-finetuning/fineweb-1m-sample" or
        corpus_spec.get("dataset", {}).get("revision") != "60b53a86b84eb6559e4407b113356f56a152318f" or
        corpus_spec.get("selection", {}).get("skip_valid_examples") != 20000 or
        corpus_spec.get("selection", {}).get("sample_count") != 4096 or
        corpus_spec.get("selection", {}).get("sequence_length") != 128):
        raise ValueError("Attempt 005 frozen corpus specification mismatch")
    if (corpus_manifest.get("format_version") != 1 or corpus_manifest.get("hash_algorithm") != "sha256" or
        corpus_manifest.get("attempt_id") != attempt_id or
        corpus_manifest.get("dataset") != corpus_spec["dataset"] or
        corpus_manifest.get("selection") != corpus_spec["selection"] or
        corpus_manifest.get("tensor") != {"shape": [4096, 128], "dtype": "torch.int64", "device": "cpu", "contiguous": True}):
        raise ValueError("Attempt 005 corpus manifest mismatch")
    for key, path, description in (
        ("corpus_spec_sha256", corpus_spec_path, "corpus specification"),
        ("freeze_script_sha256", freeze_corpus_script_path, "corpus freezer script"),
    ):
        verify_manifest_file_link(corpus_manifest, key, path, description)
    corpus_artifact = corpus_manifest.get("artifact")
    tokenizer = corpus_manifest.get("tokenizer_checkpoint")
    if (not isinstance(corpus_artifact, dict) or not isinstance(tokenizer, dict) or
        corpus_artifact.get("path") != corpus_spec["artifact"]["path"] or
        tokenizer.get("source") != "canonical_merged_checkpoint" or
        tokenizer.get("directory") != corpus_spec["tokenizer"]["directory"]):
        raise ValueError("Attempt 005 corpus artifact or tokenizer provenance mismatch")
    for key in ("serialized_sha256", "raw_tensor_sha256"):
        require_sha256(corpus_artifact.get(key), f"corpus {key}")
    if (construction.get("format_version") != 1 or construction.get("hash_algorithm") != "sha256" or
        construction.get("attempt_id") != attempt_id or
        construction.get("information_policy") != attempt_spec["information_policy"] or
        construction.get("loss", {}).get("type") != attempt_spec["generic_loss"]["type"] or
        construction.get("gradient_and_rollback", {}).get("eligible_matrix_count") != 98 or
        construction.get("gradient_and_rollback", {}).get("target_relative_frobenius") != 0.00125):
        raise ValueError("Attempt 005 construction manifest mismatch")
    for key, path, description in (
        ("attempt_spec_sha256", attempt_spec_path, "construction attempt specification"),
        ("corpus_spec_sha256", corpus_spec_path, "construction corpus specification"),
        ("corpus_manifest_sha256", corpus_manifest_path, "construction corpus manifest"),
        ("constructor_script_sha256", constructor_script_path, "constructor script"),
    ):
        verify_manifest_file_link(construction, key, path, description)
    if construction.get("corpus") != {
        "serialized_sha256": corpus_artifact["serialized_sha256"],
        "raw_tensor_sha256": corpus_artifact["raw_tensor_sha256"],
    }:
        raise ValueError("Construction corpus hash provenance mismatch")
    loss = construction["loss"]
    for key, value in attempt_spec["generic_loss"].items():
        if loss.get(key) != value:
            raise ValueError(f"Construction generic loss mismatch: {key}")
    expected_self = {
        "readout_block_index": 13, "num_hidden_layers": 28, "sample_count": 10000,
    ...
    if (self_spec.get("format_version") != 1 or self_spec.get("attempt_id") != attempt_id or
        self_spec.get("input_information_policy") !=
        "canonical_merged_attempt005_rollback_checkpoint_and_frozen_generic_probe_only"):
        raise ValueError("Attempt 005 self-difference specification mismatch")
    ...
    if (self_manifest.get("format_version") != 1 or self_manifest.get("hash_algorithm") != "sha256" or
        ...
        raise ValueError("Attempt 005 self-difference manifest mismatch")
    for key, path, description in (
        ...
        ("corpus_spec_sha256", corpus_spec_path, "self-difference corpus specification"),
        ("corpus_manifest_sha256", corpus_manifest_path, "self-difference corpus manifest"),
        ("constructor_script_sha256", constructor_script_path, "self-difference constructor script"),
    ...
    if (not isinstance(probe, dict) or
        probe != {key: self_spec["probe"][key] for key in ("serialized_sha256", "raw_tensor_sha256")}):
    ...
    sources = [normalize_checkpoint_entry(entry, "merged checkpoint") for entry in (
        construction.get("source_checkpoint"), inputs.get("merged"), merged_manifest,
    )]
    if any(source != sources[0] for source in sources[1:]):
        raise ValueError("Merged checkpoint provenance disagrees across manifests")
    merged_files = {record["path"]: record for record in sources[0]["files"]}
    tokenizer_files = normalize_file_records(tokenizer.get("files"), "corpus tokenizer checkpoint")
    if (tokenizer.get("file_count") != len(tokenizer_files) or
        any(merged_files.get(record["path"]) != record for record in tokenizer_files)):
        raise ValueError("Corpus tokenizer checkpoint disagrees with merged checkpoint")
    constructed = construction.get("output_checkpoint")
    rollback = inputs.get("rollback")
    if (not isinstance(constructed, dict) or not isinstance(rollback, dict) or
        constructed.get("directory_name") != "model" or rollback.get("directory_name") != "model" or
        normalize_checkpoint_entry(constructed, "construction output checkpoint") !=
        normalize_checkpoint_entry(rollback, "self-difference rollback checkpoint")):
        raise ValueError("Rollback checkpoint provenance mismatch")
    ...
    return {
        ...
        "corpus_spec_sha256": sha256_file(corpus_spec_path),
        "corpus_manifest_sha256": sha256_file(corpus_manifest_path),
        "freeze_corpus_script_sha256": sha256_file(freeze_corpus_script_path),
...
def verify_inputs_before_loading(
    ...
    *,
    ...
    corpus_spec_path: Path = DEFAULT_CORPUS_SPEC_PATH,
    ...
    freeze_corpus_script_path: Path = DEFAULT_FREEZE_CORPUS_SCRIPT_PATH,
    ...
    attempt = validate_attempt_provenance(
        ...
        freeze_corpus_script_path=freeze_corpus_script_path,
...
def build_evaluation_output(
    ...
    *,
    ...
    values = (
        ...
        (attempt["corpus_spec_sha256"], "corpus specification"),
        (attempt["corpus_manifest_sha256"], "corpus manifest"),
        (attempt["freeze_corpus_script_sha256"], "corpus freezer script"),
    ...
    return {
        ...
        "provenance": {
            ...
            "attempt": {
                "corpus_spec_sha256": attempt["corpus_spec_sha256"],
                "corpus_manifest_sha256": attempt["corpus_manifest_sha256"],
                "freeze_corpus_script_sha256": attempt["freeze_corpus_script_sha256"],
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        for path, expected, description in (
            ...
            (DEFAULT_CORPUS_SPEC_PATH, context["attempt"]["corpus_spec_sha256"], "Corpus specification"),
            (DEFAULT_CORPUS_MANIFEST_PATH, context["attempt"]["corpus_manifest_sha256"], "Corpus manifest"),
            (DEFAULT_FREEZE_CORPUS_SCRIPT_PATH, context["attempt"]["freeze_corpus_script_sha256"], "Corpus freezer script"),
