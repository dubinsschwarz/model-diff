...
"""Compute Attempt 003 merged-minus-noisy activation differences."""
...
FROZEN_SELF_DIFF_SPEC = {
    ...
    "input_information_policy": (
        "canonical_merged_attempt003_gaussian_noisy_checkpoint_and_frozen_generic_"
        "probe_only"
    ),
...
def validate_attempt_inputs(
    ...
    noisy_entry = verify_checkpoint(
        checkpoint_dir, output_entry, "Gaussian-noisy checkpoint"
    )
    return {
...
def build_artifact(
    ...
    validate_float64_mean(
        noisy_mean64, sequence_length, hidden_size, "Noisy mean", torch_module
    )
    difference64 = merged_mean64 - noisy_mean64
    return {
        ...
        "noisy_mean": noisy_mean64.to(torch_module.float32).contiguous(),
...
def build_manifest(
    ...
    normalized_noisy = normalize_checkpoint_entry(
        noisy_checkpoint, "noisy checkpoint manifest entry"
    )
    for value, description in (
        ...
        (raw_hashes.get("noisy_mean"), "noisy mean"),
        (raw_hashes.get("difference"), "self-difference"),
    ):
        require_sha256(value, description)
    directory_name = noisy_checkpoint.get("directory_name")
    if not isinstance(directory_name, str) or not directory_name:
        raise ValueError("Noisy checkpoint directory name is invalid")
    return {
        "format_version": 1,
        "attempt_id": spec["attempt_id"],
        ...
        "input_checkpoints": {
            ...
            "noisy": {"directory_name": directory_name, **normalized_noisy},
        },
        "probe": {
        ...
        "self_diff_artifact": {
            ...
            "raw_tensors_sha256": {
                ...
                "noisy_mean": raw_hashes["noisy_mean"],
...
def save_outputs(
    ...
    manifest = build_manifest(
        ...
        input_context["noisy_checkpoint"],
...
def compute_means(
    ...
    noisy_mean, noisy_hidden_size = compute_checkpoint_mean(
        input_context["noisy_checkpoint"]["directory"],
        probe,
        hidden_size,
        spec,
        torch_module,
    ...
    if noisy_hidden_size != hidden_size:
        raise ValueError("Noisy checkpoint hidden size mismatch")
    return merged_mean, noisy_mean, hidden_size
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        merged_mean, noisy_mean, hidden_size = compute_means(
            args.merged_model_dir, input_context, probe, spec, torch
        )
        artifact = build_artifact(
            merged_mean, noisy_mean, hidden_size, spec, torch
        )
