...
SCRIPT_PATH = PROJECT / "scripts" / "ablation" / "evaluate_attempt002.py"
SPEC_PATH = (
    PROJECT
    / "experiments"
    / "attempts"
...
MODULE_SPEC = importlib.util.spec_from_file_location("evaluate_attempt002", SCRIPT_PATH)
...
class Attempt002EvaluationTests(unittest.TestCase):
    def test_syntax_import_and_frozen_spec(self) -> None:
        ...
        self.assertEqual(spec["attempt_id"], "002_bf16_roundtrip_prefix0_13")
    ...
    def _all_keys(value):
        if isinstance(value, dict):
            for key, child in value.items():
                yield key
                yield from Attempt002EvaluationTests._all_keys(child)
