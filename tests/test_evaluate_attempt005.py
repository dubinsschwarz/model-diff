...
SCRIPT_PATH = PROJECT / "scripts" / "ablation" / "evaluate_attempt005.py"
SPEC_PATH = (
    PROJECT
    / "experiments"
    / "attempts"
...
MODULE_SPEC = importlib.util.spec_from_file_location("evaluate_attempt005", SCRIPT_PATH)
...
class Attempt005EvaluationTests(unittest.TestCase):
    @staticmethod
    def tied_weights():
        return {
    ...
    def test_committed_attempt005_hash_links(self) -> None:
        ...
        corpus_manifest = json.loads((ATTEMPT_DIRECTORY / "corpus-manifest.json").read_text())
        for manifest, links in (
            (corpus_manifest, {
                "corpus_spec_sha256": ATTEMPT_DIRECTORY / "corpus_spec.json",
                "freeze_script_sha256": evaluation.DEFAULT_FREEZE_CORPUS_SCRIPT_PATH,
            }),
            (construction, {
                "attempt_spec_sha256": ATTEMPT_DIRECTORY / "spec.json",
                "corpus_spec_sha256": ATTEMPT_DIRECTORY / "corpus_spec.json",
                "corpus_manifest_sha256": ATTEMPT_DIRECTORY / "corpus-manifest.json",
            ...
            (self_manifest, {
                ...
                "corpus_spec_sha256": ATTEMPT_DIRECTORY / "corpus_spec.json",
                "corpus_manifest_sha256": ATTEMPT_DIRECTORY / "corpus-manifest.json",
                "construction_manifest_sha256": ATTEMPT_DIRECTORY / "construction-manifest.json",
                "constructor_script_sha256": evaluation.DEFAULT_CONSTRUCTOR_SCRIPT_PATH,
                "self_diff_spec_sha256": ATTEMPT_DIRECTORY / "self_diff_spec.json",
                "compute_self_diff_script_sha256": evaluation.DEFAULT_SELF_DIFF_SCRIPT_PATH,
            }),
        ):
    ...
    def test_attempt_provenance_chain_without_loading_checkpoint(self) -> None:
        ...
        self.assertEqual(context["method"]["difference_sign"], "merged_minus_rollback")
    ...
    def test_attempt_provenance_frozen_file_tamper_fails(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            tampered.write_text(evaluation.DEFAULT_ATTEMPT_SPEC_PATH.read_text().replace(
                "generic_data_gradient_rollback", "changed_method"
            ))
    ...
    def test_synthetic_self_diff_raw_hashes_and_rollback_field(self) -> None:
        values = {
            "merged_mean": torch.zeros(evaluation.EXPECTED_SHAPE, dtype=torch.float32),
            "rollback_mean": torch.ones(evaluation.EXPECTED_SHAPE, dtype=torch.float32),
            "difference": -torch.ones(evaluation.EXPECTED_SHAPE, dtype=torch.float32),
        }
        method = {
            "readout_block_index": 13, "num_hidden_layers": 28,
            "sample_count": 10000, "sequence_length": 128, "hidden_size": 2048,
            "batch_size": 32, "model_dtype": "float32",
            "accumulator_dtype": "float64", "stored_dtype": "float32",
            "difference_sign": "merged_minus_rollback",
        }
        hashes = {
            name: evaluation.sha256_raw_float32_tensor(value, torch)
            for name, value in values.items()
        }
        context = {"attempt": {
            "method": method,
            "self_diff_manifest": {"self_diff_artifact": {"raw_tensors_sha256": hashes}},
        }}
        artifact = {
            "format_version": 1,
            "metadata": {"attempt_id": evaluation.FROZEN_EVALUATION_SPEC["attempt_id"], **method},
            **values,
        }
        with tempfile.TemporaryDirectory() as temp_dir:
            path = Path(temp_dir) / "self_diff.pt"
            torch.save(artifact, path)
            loaded = evaluation.load_and_validate_self_diff(
                path, context, evaluation.FROZEN_EVALUATION_SPEC, torch
            )
            self.assertEqual(set(loaded), {"format_version", "metadata", *values})
            artifact["difference"] = torch.zeros_like(values["difference"])
            torch.save(artifact, path)
            with self.assertRaisesRegex(ValueError, "raw tensor SHA-256 mismatch"):
                evaluation.load_and_validate_self_diff(
                    path, context, evaluation.FROZEN_EVALUATION_SPEC, torch
                )
    ...
    def _all_keys(value):
        if isinstance(value, dict):
            for key, child in value.items():
                yield key
                yield from Attempt005EvaluationTests._all_keys(child)
